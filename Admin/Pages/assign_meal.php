...
if (isset($_POST['assign_meal'])) {
    ...
    if ($calories < 0 || $protein < 0 || $carbs < 0 || $fats < 0) {
        ...
        $sql = "INSERT INTO meal_plans (user_id, meal_name, calories, protein, carbs, fats, meal_day, meal_time)
                VALUES ('$user_id', '$meal_name', '$calories', '$protein', '$carbs', '$fats', '$meal_day', '$meal_time')";
        $cn->query($sql);
...
$meals = $cn->query("SELECT m.id, u.name AS username, m.meal_name, m.calories, m.protein, m.carbs, m.fats, m.meal_day, m.meal_time 
                    FROM meal_plans m 
...
if (isset($_POST['update_meal'])) { 
    ...
    $sql = "UPDATE meal_plans SET meal_name='$meal_name', calories='$calories', protein='$protein', carbs='$carbs', fats='$fats', meal_day='$meal_day', meal_time='$meal_time' WHERE id=$id"; 
    $cn->query($sql); 
